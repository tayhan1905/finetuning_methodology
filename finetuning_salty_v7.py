"""
finetuning_salty_v7.py  —  SALTEDORA-V4 (Knee, r=64) vs Full Fine-Tuning
=========================================================================
Trains SALTEDORA-V4 with cumulative energy knee split (rank 64) and full
fine-tuning on SST-2, then computes principal angles between their weight
subspaces at each epoch and at the final checkpoint.

Knee bounds
-----------
    min_frac = 0.50  →  head gets at least 50% of singular values
    max_frac = 0.90  →  head gets at most  90% of singular values

Outputs
-------
results_v7/bert-base-uncased/full_ft/reference/glue_sst2/
    results.json  |  loss_per_batch.csv  |  loss_per_epoch.csv
    weights/epoch_<n>.pt  …  final.pt

results_v7/bert-base-uncased/saltedora_v4_knee/r_64/glue_sst2/
    results.json  |  loss_per_batch.csv  |  loss_per_epoch.csv
    weights/epoch_<n>.pt  …  final.pt

results_v7/principal_angles/
    final_comparison_summary.csv
    epoch_trajectory_summary.csv
    <layer>_trajectory_angles.npz

results_v7/summary_v7.csv
"""
...
# Knee bounds (head is the dominant subspace)
MIN_FRAC = 0.50
MAX_FRAC = 0.90

RESULTS_ROOT = "./results_v7"
...
def train_and_evaluate(model, train_ds, val_ds, tokenizer, metrics_fn,
    ...
    results["min_frac"]         = MIN_FRAC if mode != "full_ft" else None
    results["max_frac"]         = MAX_FRAC if mode != "full_ft" else None
...
def analyze_principal_angles(saltedora_dir: str, full_ft_dir: str, out_dir: str) -> None:
...
if __name__ == "__main__":
    ...
    # ---- Step 2: SALTEDORA-V4 (knee, r=64) ----
    logger.info("\n" + "="*60)
    logger.info(f"STEP 2: SALTEDORA-V4 knee  r={RANK}  min_frac={MIN_FRAC}  max_frac={MAX_FRAC}")
    logger.info("="*60)

    salty_dir = f"{RESULTS_ROOT}/{MODEL_NAME}/saltedora_v4_knee/r_{RANK}/glue_sst2"
    ...
    # ---- Step 3: Principal angles analysis ----
    logger.info("\n" + "="*60)
    logger.info("STEP 3: Principal angles analysis")
    logger.info("="*60)

    pa_dir = f"{RESULTS_ROOT}/principal_angles"
    analyze_principal_angles(salty_dir, full_ft_dir, pa_dir)

    # ---- Summary CSV ----
    summary = pd.DataFrame([
        {"mode": "full_ft",        "rank": 0,    "min_frac": None, "max_frac": None,
        ...
        {"mode": "saltedora_v4_knee", "rank": RANK, "min_frac": MIN_FRAC, "max_frac": MAX_FRAC,
    ...
    summary.to_csv(f"{RESULTS_ROOT}/summary_v7.csv", index=False)
    logger.info(f"\n✅ Done. Summary → {RESULTS_ROOT}/summary_v7.csv")
