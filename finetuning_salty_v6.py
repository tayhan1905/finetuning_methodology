...
===================================================================
Benchmarks SALTEdoraLinearV4 across four GLUE classification tasks
and five rank values, using eigen dispersion with energy_threshold=0.8.

Method
------
    • SALTEdoraLinearV4  (eigen-dispersion split, r_top_override=None)
...
Key design choice — eigen dispersion
-------------------------------------
    r_top_override   = None  →  choose_head_rank_by_eigen_dispersion() is used.
    energy_threshold = 0.8   →  fallback threshold (lower than v3's 0.9, so the
                                 curvature heuristic has a tighter spectral budget
                                 before it reverts to the energy criterion).
...
# Adapter replacement
# ===========================================================================
def replace_qkv_with_adapter(model, r: int):
    """
...
def train_and_evaluate(
    ...
    model_name: str, r: int, task_key: str,
    num_labels: int, problem_type: str,
) -> dict:
    safe_task  = task_key.replace("/", "_")
    output_dir = f"./results_v6/{model_name}/{MODE}/r_{r}/{safe_task}"
...
if __name__ == "__main__":
    ...
    summary_rows = []
    total_runs   = len(TASKS) * len(RANKS)
    run_idx      = 0

    # Outer loop: task → rank
    # (group by task so the dataset stays in memory across rank combos)
    for task_key in TASKS:
        ...
        for r in RANKS:
            ...
            model = replace_qkv_with_adapter(base_model, r=r)

            results = train_and_evaluate(
                model, train_ds, val_ds, tokenizer, metrics_fn,
                MODEL_NAME, r, task_key, num_labels, problem_type,
            )
    ...
    out_csv    = "./results_v6/summary_v6_saltedora_sweep.csv"
